...
export default function RegisterPage() {
  ...
  const [clinicForm, setClinicForm] = useState({
    clinic_name: '', clinic_type: 'pet_grooming', description: '',
  ...
  // If already authed + confirmed but no profile → salon setup
  useEffect(() => {
    if (user && user.email_confirmed_at && !profile) setStep(3);
  ...
  const handleSignUp = async (e) => {
    ...
    // If email already confirmed (e.g. Supabase has confirm disabled), skip OTP
    ...
    <div data-testid="register-page" className="min-h-screen flex" style={{ fontFamily: 'Plus Jakarta Sans, system-ui, sans-serif' }}>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 bg-white">
        <div className="w-full max-w-[480px]">
          ...
          {step === 3 && (
            <form onSubmit={handleSetupClinic} className="space-y-5">
              <div>
                <Label className="text-sm font-medium text-gray-700">Salon Name *</Label>
                <Input data-testid="register-clinic-name-input" value={clinicForm.clinic_name} onChange={e => updateClinic('clinic_name', e.target.value)} placeholder="Pawfect Grooming Salon" required className="mt-1.5 h-11 rounded-lg border-gray-200 focus:border-indigo-400 focus:ring-indigo-400/20" />
              </div>
              <div>
                <Label className="text-sm font-medium text-gray-700">Description</Label>
                <Input value={clinicForm.description} onChange={e => updateClinic('description', e.target.value)} placeholder="A brief tagline for your salon" className="mt-1.5 h-11 rounded-lg border-gray-200 focus:border-indigo-400 focus:ring-indigo-400/20" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm font-medium text-gray-700">Salon Email</Label>
                  <Input value={clinicForm.email} onChange={e => updateClinic('email', e.target.value)} type="email" placeholder="hello@salon.com" className="mt-1.5 h-11 rounded-lg border-gray-200 focus:border-indigo-400 focus:ring-indigo-400/20" />
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-700">Phone</Label>
                  <Input value={clinicForm.phone} onChange={e => updateClinic('phone', e.target.value)} placeholder="+61 4XX XXX XXX" className="mt-1.5 h-11 rounded-lg border-gray-200 focus:border-indigo-400 focus:ring-indigo-400/20" />
                </div>
              </div>
              <div>
                <Label className="text-sm font-medium text-gray-700">Address</Label>
                <Input value={clinicForm.address} onChange={e => updateClinic('address', e.target.value)} placeholder="123 Main Street" className="mt-1.5 h-11 rounded-lg border-gray-200 focus:border-indigo-400 focus:ring-indigo-400/20" />
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <Label className="text-sm font-medium text-gray-700">City</Label>
                  <Input value={clinicForm.city} onChange={e => updateClinic('city', e.target.value)} placeholder="Sydney" className="mt-1.5 h-11 rounded-lg border-gray-200 focus:border-indigo-400 focus:ring-indigo-400/20" />
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-700">State</Label>
                  <Input value={clinicForm.state} onChange={e => updateClinic('state', e.target.value)} placeholder="NSW" className="mt-1.5 h-11 rounded-lg border-gray-200 focus:border-indigo-400 focus:ring-indigo-400/20" />
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-700">Postal Code</Label>
                  <Input value={clinicForm.postal_code} onChange={e => updateClinic('postal_code', e.target.value)} placeholder="2000" className="mt-1.5 h-11 rounded-lg border-gray-200 focus:border-indigo-400 focus:ring-indigo-400/20" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm font-medium text-gray-700">Country</Label>
                  <Input value={clinicForm.country} onChange={e => updateClinic('country', e.target.value)} placeholder="Australia" className="mt-1.5 h-11 rounded-lg border-gray-200 focus:border-indigo-400 focus:ring-indigo-400/20" />
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-700">Timezone</Label>
                  <Input value={clinicForm.timezone} onChange={e => updateClinic('timezone', e.target.value)} placeholder="Australia/Sydney" className="mt-1.5 h-11 rounded-lg border-gray-200 focus:border-indigo-400 focus:ring-indigo-400/20" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4 items-start">
                <div>
                  ...
                  <div className="mt-1.5 flex items-center gap-3">
                    {clinicForm.logo_url ? (
                      <img src={clinicForm.logo_url} alt="Logo" className="h-11 w-11 rounded-lg object-cover border" />
                    ) : (
              ...
              {error && <p data-testid="register-error" className="text-sm text-red-600 bg-red-50 rounded-lg p-3">{error}</p>}
              <Button data-testid="register-clinic-submit-btn" type="submit" disabled={loading} className="w-full rounded-lg h-11 bg-gray-900 text-white hover:bg-gray-800 text-sm font-semibold">
